    ...
    try {
      ...
      if (user != null) {
        ...
        final stats = FirestoreService.calculateStats(
            weight, height, currentAge, gender, activityLevel, goal);
      ...
      if (mounted) {
        ...
        if (e.code == 'permission-denied') {
          message =
              'ไม่มีสิทธิ์บันทึกข้อมูล (Permission Denied) กรุณาตรวจสอบ Security Rules';
        }
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('เกิดข้อผิดพลาดไม่ทราบสาเหตุ: $e'),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
              color: selected ? Colors.blueAccent : Colors.blue[50]!),
          boxShadow: [
            if (selected)
              BoxShadow(
